...
log(
    f"loaded {time.perf_counter() - t0:.0f}s: train {len(train):,}  "
...
p = st.profile
log(
    f"fit {time.perf_counter() - t0:.0f}s  base={p.get('base_scorer_used')}  "
...
log(
    f"BOOK-CHRONO: NDCG={rep.ndcg_at_k:.4f}  MRR={rep.mrr:.4f}  "
